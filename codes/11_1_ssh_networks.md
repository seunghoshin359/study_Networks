...
```shell
[fedora@192.168.0.49 ~/quests]$ sh server.sh
server.sh: line 9: [: ti: integer expression expected
good 


[fedora@192.168.0.49 ~/quests]$ cat server.sh
#!bin/bash

ping -c 1 192.168.0.53 > pingTest.txt
Q=$(cat pingTest.txt | cut -d , -f 2 | grep -i " 1 received")
T=$(tail -n 2 pingTest.txt | cut -d , -f 4 | cut -d " " -f 2 | cut -d "m" -f 1 )
aaa=" 1 received"
if [ "$Q" = "$aaa"  ]; then
